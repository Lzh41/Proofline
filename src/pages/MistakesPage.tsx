        ...
        <section className={styles.codeWorkbench}>
          ...
          <div className={styles.solveEditor} key={`${problem.id}:${language}`}>
            <Suspense fallback={<div className={styles.notice} style={{ margin: 16 }}>正在加载本地编辑器…</div>}>
              <MonacoEditor height="100%" language={language === 'cpp' ? 'cpp' : language} defaultValue={codeRef.current} theme={editorTheme} options={{ fontSize: editorFontSize, fontFamily: 'JetBrains Mono, Consolas, monospace', minimap: { enabled: false }, scrollBeyondLastLine: false, automaticLayout: true, padding: { top: 16 }, wordWrap: 'off', codeLens: false, folding: false, occurrencesHighlight: 'off', selectionHighlight: false, wordBasedSuggestions: 'off', tabCompletion: 'on', autoClosingBrackets: 'languageDefined', autoClosingQuotes: 'languageDefined', autoIndent: 'advanced', renderValidationDecorations: 'off', smoothScrolling: true, cursorSmoothCaretAnimation: 'on', cursorBlinking: 'smooth', renderLineHighlight: 'gutter', scrollbar: { verticalSliderSize: 8, horizontalSliderSize: 8 }, stickyScroll: { enabled: false }, bracketPairColorization: { enabled: false }, guides: { bracketPairs: false, indentation: true }, quickSuggestions: false }} onChange={handleCodeChange} onMount={handleEditorMount} />
            </Suspense>
          </div>
