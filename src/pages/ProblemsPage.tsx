  ...
  return (
    <>
      ...
      <div className={`${styles.filters} ${styles.problemFilters}`}>
        <label className="field"><span className="srOnly">搜索题目</span><div style={{ position: 'relative' }}><Search size={15} style={{ position: 'absolute', left: 11, top: 12, color: 'var(--muted)' }} /><input id="problem-search" className="input" style={{ paddingLeft: 34 }} value={query} onChange={(event) => setSearchParams(event.target.value ? { q: event.target.value } : {})} placeholder="标题、标签或题号" /></div></label>
